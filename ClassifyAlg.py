...
import numpy as np
data = [570.604454093,565.256777185,545.471744387,528.741260761,460.439789868]
...
def _scoreAlg(activity,logReader):
    ...
    if flag:
        return "*"+str(label[0]),activity,Matrix[:,1:]
    else:
        return "*"+str(random.randint(1,5)),activity,None
def classifyAlg(activity, logReader):
    a,b      = _scoreAlg(activity,logReader)
    grade       = int(a[1:])
    review      = ""
    if grade    >=8:
        grade   = 3
